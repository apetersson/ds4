...
.PHONY: all help clean test test-rocm test-glm53-kda-rocm test-metal-session-batch test-mxfp4-cuda test-mxfp4-rocm test-cuda-session-batch test-cuda-mixed-batch test-imatrix-inputs dspark-acceptance dspark-verify-depth mtp-verify-depth cpu cuda cuda-spark cuda-generic cuda-regression strix-halo rocm
...
test-rocm:
	$(MAKE) -B ds4_test ds4_agent_test ds4-eval q4k-dot-test mxfp4-dot-test \
		tests/test_layer_pack tests/test_engine_mgpu_placement tests/test_gpu_args tests/test_imatrix_inputs tests/test_prompt_prefix \
...
tests/test_imatrix_inputs.o: tests/test_imatrix_inputs.c ds4.h
	$(CC) $(CFLAGS) -DDS4_TEST_HOOKS -I. -c -o $@ $<

tests/test_imatrix_inputs: tests/test_imatrix_inputs.o ds4_cpu_test_hooks.o ds4_image.o ds4_distributed.o ds4_tp.o ds4_ssd.o ds4_layer_pack.o
	$(CC) $(CFLAGS) -o $@ $^ $(LDLIBS)

test-imatrix-inputs: tests/test_imatrix_inputs
	./tests/test_imatrix_inputs
