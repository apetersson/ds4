...
int main(void) {
    ...
    }
    {
        ...
        CHECK("DSpark without explicit or catalog support is rejected",
              !ok && strstr(err, "requires either explicit --mtp-model") != NULL);
    }
    {
        char *argv[] = {"ds4", "--hf", "owner/repo"};
        ds4_hf_cli_config cfg;
