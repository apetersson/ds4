...
static void server_image_inputs_free(server_image_inputs *images) {
    ...
    for (size_t i = 0; i < images->len; i++) {
        free(images->v[i].encoded);
        free(images->v[i].data_uri);
    }
    free(images->v);
...
static bool server_image_inputs_push_data_uri(
    ...
    if (!strncmp(uri, png, sizeof(png) - 1))
        ok = server_image_inputs_push_base64(
            images, "image/png", uri + sizeof(png) - 1, marker);
    else if (!strncmp(uri, jpeg, sizeof(jpeg) - 1))
        ok = server_image_inputs_push_base64(
            images, "image/jpeg", uri + sizeof(jpeg) - 1, marker);
    else if (!strncmp(uri, jpg, sizeof(jpg) - 1))
        ok = server_image_inputs_push_base64(
            images, "image/jpg", uri + sizeof(jpg) - 1, marker);
    if (ok) images->v[images->len - 1].data_uri = xstrdup(uri);
    return ok;
}
...
static DS4_SERVER_MAYBE_UNUSED bool json_message_content_replace(
        const char **p, char **dst, char **image_url, int *image_count) {
    char *tmp = NULL;
...
static bool replace_openai_image_marker(char **text, const char *marker,
                                        const char *replacement) {
    if (!text || !*text || !marker || !marker[0]) return false;
    const char *mark = strstr(*text, marker);
    if (!mark || strstr(mark + strlen(marker), marker)) return false;
    const size_t before = (size_t)(mark - *text);
    const size_t after = strlen(mark + strlen(marker));
    const size_t replacement_len = strlen(replacement);
    const size_t before_sep = before != 0 ? 2u : 0u;
    const size_t after_sep = after != 0 ? 2u : 0u;
    char *out = xmalloc(before + before_sep + replacement_len + after_sep + after + 1);
    size_t pos = 0;
    memcpy(out + pos, *text, before);
    pos += before;
    if (before_sep) {
        memcpy(out + pos, "\n\n", before_sep);
        pos += before_sep;
    ...
    memcpy(out + pos, mark + strlen(marker), after + 1);
    free(*text);
    *text = out;
    return true;
}
...
static bool stage_openai_image(request *r, chat_msgs *msgs,
    ...
    for (int i = 0; i < msgs->len; i++) {
        for (size_t j = 0; j < msgs->v[i].images.len; j++) {
            if (replace_openai_image_marker(&msgs->v[i].content,
                                            msgs->v[i].images.v[j].marker,
                                            DS4_IMAGE_TOKEN)) {
                replacements++;
            }
        }
...
static bool parse_chat_request(ds4_engine *e, server *s,
    ...
    if (vision_config_complete(vision)) {
        size_t external_images = 0;
        const char *external_uri = NULL;
        for (int i = 0; i < msgs.len; i++) {
            for (size_t j = 0; j < msgs.v[i].images.len; j++) {
                external_images++;
                external_uri = msgs.v[i].images.v[j].data_uri;
            }
        }
        if (external_images > 1) {
            snprintf(err, errlen, "external vision sidecars support exactly one image per request");
            chat_msgs_free(&msgs);
            free(tool_schemas);
            request_free(r);
            return false;
        ...
        if (external_images == 1) {
            if (!external_uri) {
                snprintf(err, errlen, "external vision sidecar requires an image data URI");
                chat_msgs_free(&msgs);
                free(tool_schemas);
                request_free(r);
                return false;
            }
            r->image_url = xstrdup(external_uri);
    ...
    if (r->image_url) {
        ...
        r->prompt_text = render_chat_prompt_text_for_syntax(
            ...
            &r->tool_orders, r->think_mode);
        ds4_tokenize_rendered_chat(e, r->prompt_text, &r->prompt);
    } else if (!request_tokenize_multimodal_prompt(e, s, r, &msgs,
                                                    err, errlen)) {
        chat_msgs_free(&msgs);
        free(tool_schemas);
        request_free(r);
...
static void generate_job_inner(server *s, server_slot *slot, job *j) {
    ...
    const bool vision_request = multimodal || has_embedding_span;
    ...
    ds4_session_set_progress(slot->session, NULL, NULL);
    ds4_session_set_display_progress(slot->session, NULL, NULL);
    if (!vision_request) kv_cache_maybe_store_continued(s, slot);
    server_log(DS4_LOG_PREFILL,
    ...
    if (cold_store_len == prompt_for_sync->len) {
        if (!vision_request && kv_cache_store_live_prefix(s, slot, prompt_for_sync,
                                       cold_store_len, "cold")) {
            kv_cache_slot_note_store(slot, cold_store_len);
            suppressed_continued_last = -1;
        } else {
...
static void test_openai_image_content_preserves_order(void) {
    ...
    TEST_ASSERT(msgs.v[0].images.v[0].data_uri != NULL &&
                !strcmp(msgs.v[0].images.v[0].data_uri,
                        "data:image/png;base64,AA=="));

    buf expected = {0};
    buf_puts(&expected, "before ");
    buf_puts(&expected, msgs.v[0].images.v[0].marker);
    ...
    r.image_url = xstrdup(msgs.v[0].images.v[0].data_uri);
    char err[128] = {0};
...
static void test_openai_image_staging_finds_rendered_start(void) {
    ...
    user.role = xstrdup("user");
    char marker[SERVER_IMAGE_MARKER_BYTES];
    TEST_ASSERT(server_image_inputs_push_data_uri(
        &user.images, r.image_url, marker));
    buf content = {0};
    buf_puts(&content, "before ");
    buf_puts(&content, marker);
